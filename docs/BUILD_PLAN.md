...
- [x] `/golden-bracket` wizard (straight into bracket → scorer → review, no separate intro screen; swap-on-conflict slot picker, Boot-race-sorted scorer list with goals/apps, live point values, editable-until-lock) + `submitGoldenBracket` action (window re-derived server-side, temporal lock, full validation). The submitted/locked summary carries a **Share my bracket** button (clipboard digest via `buildGoldenBracketShareText`) and a **Place your match bets →** link back to `/today` (the bracket is a side bet — the summary sends you on to the day's slate), plus Edit-until-lock.
- [x] `/today` promo above the match list in betting/allset/settling states: full CTA → compact "bracket's in" → post-lock "view" (hidden if never placed). A **compact** variant of the same promo also leads into the knockouts on `/fixtures` (derived from the loaded fixtures, no extra `matches` read).
- [x] `/fixtures` polish shipped alongside: QF/SF/Final rows emphasised (gold border + wash, round-name badge), proper round labels via `src/lib/stage.ts`, an instant jump-to-next-match on open (`ScrollToAnchor`, offset for the sticky nav), and **placeholders for undrawn knockout slots** (dashed "teams TBD" cards from the fixed WC2026 calendar in `src/lib/knockout-schedule.ts`, merged + deduped by the unit-tested `src/lib/schedule.ts`). The Golden Bracket banner shows a **pre-window teaser** ("opens once the quarter-finals are set") before the QF field is complete, then the live promo.
- [x] `/admin` gallery: "Golden Bracket" group (full + compact promo states + all wizard steps + submitted/locked) on mock data with the real odds table; the Schedule preview carries mock QF/SF/Final rows + the compact promo.
