...
- [x] `/fixtures` polish shipped alongside: QF/SF/Final rows emphasised (gold border + wash, round-name badge), proper round labels via `src/lib/stage.ts`, and an instant jump-to-next-match on open (`ScrollToAnchor`, offset for the sticky nav).
