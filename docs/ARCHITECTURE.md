...
| `squads-sync` | Daily, 07:30 Helsinki (04:30 UTC) | Vercel Cron (`vercel.json`) | Pull each team's 26-man list; mark players dropped from the official list as `out`. |
| `af-map` | Daily, 05:00 Helsinki (02:00 UTC) | Vercel Cron (`vercel.json`) | Re-run the API-Football ID mapping (teams/fixtures/players) so newly-scheduled knockout fixtures get an `af_fixture_id` **before** they're played and settled. Idempotent (only sets `af_*` ids where a unique match is found). Runs before the morning `settle` sweep so a match played the previous evening is already mapped — the fix for the July 4 Paraguay–France gap where the R16 pairing, set after the last manual mapping run, settled unmapped and voided Mbappé's penalty. |
...
**Tournament-end settlement (Golden Bracket, migration `016`).** `runSettlement` ends with `settleGoldenBracket()`, a no-op every tick until the tournament is truly over: the `final` must be finished **and** settled, and no non-void match may remain unfinished or unsettled — which guarantees every goal event has been ingested, so the top-scorer tally is complete. (The explicit final gate matters: mid-tournament there are moments when every *existing* row is settled but the next round's fixtures aren't drawn yet.) It then resolves placements from the sf/third/final rows (`resolvePlacements`), the tied-or-sole scorer leaders from `match_events` (`topScorers`), and pays each winning line as one idempotent season-keyed ledger row (`gb_*`) — pure math in `src/settlement/golden-bracket.ts`, shared with the wizard UI so displayed and paid values can't drift.
...
- ⚠️ **`m.stage` is not trustworthy for the expanded 48-team knockout rounds.** football-data's stage literal for the new round-of-32 didn't match the `ROUND_OF_32` mapping in `mapStage()` (`fixtures-sync/route.ts`), so all 16 r32 fixtures silently fell through to the `?? 'group'` default — no `group` label, but tagged `stage: 'group'` — which meant the favorite-team "out of group" milestone (`settlement/favorites.ts`) never fired for anyone. The WC2026 stage *windows* are fixed and published well ahead of the tournament — only the teams in each slot are unknown beforehand — so `resolveStage()` now derives the stage from `kickoff_at` against a hardcoded boundary table for any fixture with no `group`, and only uses `mapStage()` as a logged cross-check. **The boundaries must sit in the GAP between rounds, not at midnight UTC** — a knockout's UTC kickoff spills past midnight for a North American evening game, so a midnight cut misfiles the round's last match. (June 30: discovered after a manager's favorite team won its r32 match and got 0 Points for it; backfilled the 16 mistagged rows' `stage`/`glory_multiplier` and the 6 missing `team_r32` ledger awards by hand. **July 8: the r16 boundary was at `2026-07-04T00:00:00Z`, but the last R32 fixture — Colombia–Ghana — kicks off `2026-07-04T01:30:00Z` (evening of Jul 3 local) and was swept into r16, inflating r16 to 9 / leaving r32 at 15 and, combined with a shootout whose winner the feed never published (Switzerland 0-0 Colombia), leaving the 4th QF fixture undrawn so the Golden Bracket window couldn't open. Moved the r16 boundary to noon UTC Jul 4; backfilled the mistagged stage, the missing shootout `winner_team_id`, and the 4th QF row by hand.**)
- ⚠️ **The free tier does NOT provide goalscorers, cards, or lineups for WC2026.** Verified 2026-06-14 against a `FINISHED` match: `/matches/{id}` returns `goals: []`, `bookings: []`, and empty `lineup` even days later — not a delay, a plan limitation. So first/anytime-scorer, carded, and lineup/appearance data **cannot** settle from this source — that's why **API-Football** (below) is now the granular provider. football-data stays the schedule/score backbone. When a match isn't yet mapped to API-Football the engine still **voids** scorer/card props (refund, no Glory) rather than falsely marking them lost. The scorer **grace-window defer** in `settleMatch` only applies to matches that actually have an API-Football mapping (`af_fixture_id` set): a football-data-only match's goal feed will *never* land, so deferring it can only ever expire — and because the 8h window outlasts the morning cron's last run (10:00 Helsinki / 07:00 UTC) for any post-~23:00-UTC kickoff, no cron tick ever exits the wait, leaving the match unsettled and its slate's recap silently blocked for everyone until some on-read nudge fires hours later. Unmapped matches therefore settle immediately, voiding their unwinnable scorer props. **This "unmapped ⇒ void" path is only safe when every real fixture is actually mapped in time** — a mapping gap turns it into silent point loss. Because the knockout bracket's pairings are only known once the prior round finishes, the `af-map` sync now runs **daily** (before the morning `settle` sweep) so a fixture is mapped as soon as its teams are set, rather than depending on someone re-running the one-off sync. As a backstop, `settleMatch` logs a loud `console.error` whenever it is about to void scorer props on an unmapped match with goals (it used to do this silently). (June 13: Brazil 1–1 Morocco settled with zero scorers, voiding two correct Vinícius anytime picks — the trigger for this whole guard, now fixed by the API-Football integration. June 30: a free-tier 4–4 with one scorer prop hung unsettled past the cron window and blocked the whole night's recap — the trigger for gating the defer on `af_fixture_id`. **July 4: Paraguay 0–1 France R16 settled unmapped — the R16 pairing was set after the last manual `af-map` run — voiding four correct Mbappé penalty picks and one favorite-player goal bonus; the trigger for scheduling `af-map` daily + the loud log.**)
...
- ⚠️ **Shootout kicks arrive as ordinary Goal/Penalty events.** AF tags a penalty-shootout kick `type: 'Goal', detail: 'Penalty'` (or `'Missed Penalty'`), distinguishable **only** by `comments: 'Penalty Shootout'`. The event mapper (`afEventToRow`, now pure in `src/settlement/af-events.ts` + unit-tested) skips them — otherwise every shootout inflates `match_events` with phantom goals, corrupting player form, favorite-player bonuses and the Golden Bracket's top-scorer tally. (July 6: found while building the Golden Bracket — the Egypt–Australia and Netherlands–Morocco R32 shootouts had stored 6 and 5 shootout kicks as `penalty` goals, including a phantom Mohamed Salah goal; repaired via `settle-backfill?match=<id>` after deploying the fix, which re-ingests events with the corrected mapper.)
...
Built (re-runnably) by the **`af-map` cron route** (`src/lib/af-mapping.ts`): teams match by name (with an alias table for Korea Republic↔South Korea, IR Iran↔Iran, …), fixtures by unordered team-pair + kickoff date, players by normalized name within each team (last-name fallback). It runs **daily** via `vercel.json` so knockout fixtures get mapped as their pairings are set (see the cron table); it's idempotent, so the schedule only ever fills in newly-resolvable ids. When adding new team-name aliases, run `af-map?dry=1` first and eyeball the unmatched lists before committing. Migration `012` adds the three `af_*` columns. This is the single most annoying piece of plumbing; the dry-run report is how you tame it.
