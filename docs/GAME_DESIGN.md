...
- **The finale recap.** The recap on the slate carrying the **final** (`buildRecap` detects a `final`-stage member) is a special variant, `finale: true`. Because the bracket settles as one season-scoped lump *at* tournament end, its Points would otherwise sit in both the standings `before` and `after` and cancel to an invisible zero delta. The finale folds each manager's `gb_*` total into the slate delta, so the leaderboard scene becomes the centrepiece: it reveals **every** manager's night — final-day Points **and** Golden Bracket Points broken out per row (not just your own) — counts each whole-tournament total up, crowns the league winner, and re-themes the title (the World Cup champion) and send-off (→ `/leaderboard` via `markFinaleSeen`). The per-slip reveal of your own final bets is unchanged; the emphasis is purely additive on the board.
- **Bracket reveal.** Before the points scene, the finale plays a dedicated **Golden Bracket slip** — *your* bracket revealed pick-by-pick exactly like a match slip (champion → fourth, then the top-scorer call and its goal-tally bonus), each line stamped HIT / TOP 4 (consolation) / MISS with its Points counting up, tallying to the bracket total. It's built in `buildRecap` (`buildGbBreakdown`) from the manager's own `gb_*` ledger rows — Points read from the ledger (never recomputed, so it respects a `settle-backfill`), hit/consolation/miss from their `golden_brackets` picks. Shown only when the manager placed a bracket.
