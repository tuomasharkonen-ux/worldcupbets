...
describe('teamLadderDeltas', () => {
  ...
  test('reaching a stage awards that rung once the fixture exists', () => {
    const matches = [
      lm({ stage: 'r32', status: 'scheduled' }),
      lm({ stage: 'r16', status: 'scheduled', away_team_id: 'T', home_team_id: 'Y' }),
    ];
    const reasons = teamLadderDeltas({ ...base, matches }).map(d => d.reason).sort();
    expect(reasons).toEqual(['team_r16', 'team_r32']);
  });

  test('champion awarded only on a finished final the team won', () => {
    const matches = [lm({ stage: 'final', status: 'finished', winner_team_id: 'T' })];
    const ds = teamLadderDeltas({ ...base, matches });
    // reaching the final (+75) AND winning it (+90)
  ...
  test('penalty-decided final: winner read from winner_team_id, not the level score', () => {
    const matches = [
      lm({ stage: 'final', status: 'finished', winner_team_id: 'T' }), // 1–1 on pens, say
    ];
    expect(teamLadderDeltas({ ...base, matches }).some(d => d.reason === 'team_champion')).toBe(true);
    // runner-up gets the final rung but not champion
    const loser = teamLadderDeltas({ ...base, teamId: 'X', matches });
  ...
  test('multiplier scales the rung amount', () => {
    const matches = [lm({ stage: 'qf', status: 'scheduled' })];
    expect(teamLadderDeltas({ ...base, multiplier: 3.5, matches })[0].amount).toBe(Math.round(35 * 3.5));
  });
