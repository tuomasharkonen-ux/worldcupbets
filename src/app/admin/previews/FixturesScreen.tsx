...
export function FixturesScreen({ empty = false }: { empty?: boolean }) {
  const { upcoming: allUpcoming, results: allResults, betsPerMatch, now } = fixturesData();
  const upcoming = empty ? [] : allUpcoming;
  const results = empty ? [] : allResults;
        ...
        <section className="space-y-3">
          ...
          {upcoming.length === 0 ? (
            ...
            <div className="space-y-2.5">
              {upcoming.map(m => (
                <MatchCard key={m.id} m={m} isLocked={now >= new Date(m.kickoff_at)} />
              ))}
            </div>
          )}
        </section>
        ...
        {results.length > 0 && (
          <section className="space-y-3">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="font-display text-lg font-bold tracking-tight text-muted">Recent results</h2>
            </div>
            <div className="space-y-2.5">
              {results.map(m => (
